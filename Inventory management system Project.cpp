...
void displayOrderStatus(Connection* conn, int orderId, int customerId) {
    ...
    try {
        ...
        stmt->executeUpdate();
        orderBelongsToCustomer = stmt->getInt(3);

        if (orderBelongsToCustomer == 0) {
...
void cancelOrder(Connection* conn, int orderId, int customerId) {
    ...
    try {
        ...
        case 3:
            ...
            break;
        default:
            cout << "Unknown error occurred while canceling the order." << endl;
            break;
        }
...
int addToCart(Connection* conn, struct ShoppingCart cart[]) {
    ...
    for (int i = 0; i < 5 && addMore == 1; i++) {
        do {
            ...
            //store the returnning value in the price variable
